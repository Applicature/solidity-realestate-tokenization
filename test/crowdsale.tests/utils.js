...
function receiptShouldSucceed(result) {
    return new Promise(function(resolve, reject) {
        let receipt = web3.eth.getTransaction(result.tx);
...
function checkClaimedTokensAmount(
    instance, 
    offsetDate, 
    lastClaimedAt, 
    currentTime, 
    ...
    return instance
        .calculateEmissionTokens(
            offsetDate + lastClaimedAt, 
            offsetDate + currentTime, 
            currentBalance, totalSupply
        )
        .then(function() {
            return instance.calculateEmissionTokens.call(
                offsetDate + lastClaimedAt, 
                offsetDate + currentTime, 
                currentBalance, 
                totalSupply
...
function checkEtherBalance(_address, expectedBalance) {
    let balance = web3.eth.getBalance(_address);
...
async function checkStateMethod(
    contract, 
    contractId, 
    stateId, 
    args
    ...
    if(Array.isArray(args)) {
        ...
        if(keys.length == 1) {
            ...
            assert.equal(
                val, 
                args[keys[0]],
                `Contract ${contractId} state ${stateId}` +
                ` with arg ${keys[0]} & value ${val} is ` +
                `not equal to ${args[keys[0]]}`
            );
        ...
        assert.equal(
            ...
            args["__val"], 
            `Contract ${contractId} state ${stateId}` + 
            `with value ${val} is not equal to ${args['__val']}`
        );
    }
    else {
        ...
        assert.equal(
            ...
            args, 
            `Contract ${contractId} state ${stateId}` +  
            `with value ${val} is not equal to ${args.valueOf()}`
        );
    }
}
